            ...
            .then(res => {
                ...
                Promise.all([
                    ...
                    .then(([peerRes, attandRes]) => {
                        ...
                        // 완료한 개수와 전체 개수 계산
                        const totalCompleted = mergedList.reduce((sum, emp) => sum + (emp.self_cnt || 0) + (emp.peer_cnt || 0), 0);
                        const totalTasks = mergedList.reduce((sum, emp) => sum + (emp.self_total_cnt || 0) + (emp.peer_total_cnt || 0), 0);
